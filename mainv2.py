...
def calculate_all_features(landmarks):
    ...
    heel_left = [landmarks[mp_pose.PoseLandmark.LEFT_HEEL_INDEX.value].x,
                landmarks[mp_pose.PoseLandmark.LEFT_HEEL_INDEX.value].y]
    ...
    heel_right = [landmarks[mp_pose.PoseLandmark.RIGHT_HEEL_INDEX.value].x,
                landmarks[mp_pose.PoseLandmark.RIGHT_HEEL_INDEX.value].y]
...
def calculate_squat_features(landmarks):
    ...
    # Foot/Shin Angle
    features[0] = calculate_angle_shift(foot_left, heel_left, ankle_left, knee_left)
    features[1] = calculate_angle_shift(foot_right, heel_right, ankle_right, knee_right)

    # Shin/Femur Angle
    features[2] = calculate_angle(ankle_left, knee_left, hip_left)
    features[3] = calculate_angle(ankle_right, knee_right, hip_right)

    # Femur/Torso Angle
    features[4] = calculate_angle(knee_left, hip_left, shoulder_left)
    features[5] = calculate_angle(knee_right, hip_right, shoulder_right)

    # Foot/Torso Angle
    features[6] = calculate_angle_shift(foot_left, heel_left, hip_left, shoulder_left)
    features[7] = calculate_angle_shift(foot_right, heel_right, hip_right, shoulder_right)

    # Foot/Femur Angle
    features[8] = calculate_angle_shift(foot_left, heel_left, knee_left, hip_left)
    features[9] = calculate_angle_shift(foot_right, heel_right, knee_right, hip_right)

    # Femur/Torso Angle
    features[10] = calculate_distance(ankle_left, ankle_right)
    features[11] = calculate_angle_shift(foot_left, heel_left, heel_right, foot_right)
...
class VideoPoseDetect(Image):
    ...
    def update(self, dt):
	    ...
	    # Draw pose prediction.
            ...
            cv2.putText(self.image, str(self.repetitions_count),
                        (int(10/640*frame_width), 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (200, 200, 0), 1, cv2.LINE_AA)

            #output_frame = cv2.cvtColor(np.array(output_frame), cv2.COLOR_RGB2BGR)
            #self.image = output_frame
